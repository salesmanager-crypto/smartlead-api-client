...
/**
 * JSON with one array element (and, at the top level, one object key) per line, so no
 * line is huge: the Claude task must be able to read the whole page before publishing it.
 */
function emit(v, depth) {
  const pad = "  ".repeat(depth + 1), end = "  ".repeat(depth);
  if (Array.isArray(v) && v.length && depth < 2) return "[\n" + v.map((x) => pad + emit(x, depth + 1)).join(",\n") + "\n" + end + "]";
  if (v && typeof v === "object" && !Array.isArray(v) && depth === 0 && Object.keys(v).length) {
    return "{\n" + Object.keys(v).map((k) => pad + json(k) + ": " + emit(v[k], depth + 1)).join(",\n") + "\n" + end + "}";
  }
  return json(v);
}
...
export function artifactDataBlock(files, pulledAt = new Date().toISOString()) {
  ...
  const lines = DASHBOARD_CONSTANTS.map((n) => `const ${n} = ${emit(d[n], 0)};`);
