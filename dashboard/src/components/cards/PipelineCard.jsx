    ...
    >
      <div className="flex h-full flex-col gap-3">
        ...
        >
          <div className="flex h-9 w-full overflow-hidden rounded-lg border border-line/20 dark:border-white/10">
            {byStage.map((s) => (
              <button
                ...
                className="group relative h-full min-w-[3%] transition-[filter] hover:brightness-110"
              >
                {s.count > 0 && (
          ...
          {hovering && (
            <div className="absolute left-0 right-0 top-11 z-10 rounded-xl border border-line/20 bg-white/95 p-2 shadow-card backdrop-blur dark:border-white/10 dark:bg-canvas/95">
              ...
              <div className="h-20 w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={correlation} margin={{ top: 4, right: 8, left: 8, bottom: 0 }}>
                    ...
                    <Area type="monotone" dataKey="outreach" stroke="#E51958" fill="url(#corrFill)" strokeWidth={2} />
                  </AreaChart>
                </ResponsiveContainer>
              </div>
            </div>
        ...
        <ul className="grid flex-1 grid-cols-2 gap-x-4 gap-y-2 content-start overflow-auto pt-2 sm:grid-cols-3">
          {byStage.map((s) => (
            <li key={s.stage}>
              <button
                onClick={() => openDrawer("deal", { stage: s.stage, deals: deals.filter((d) => d.stage === s.stage) })}
                className="flex w-full items-center gap-2 rounded-lg px-1.5 py-1 text-left hover:bg-mist dark:hover:bg-white/5"
              >
