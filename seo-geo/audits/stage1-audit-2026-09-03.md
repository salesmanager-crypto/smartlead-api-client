...
I also attempted to check the WordPress trash directly, at the user's request. Result: this session's WordPress REST API access remains anonymous and read-only. `/wp-json/wp/v2/users/me` returns `401 rest_not_logged_in`, and querying any post status other than `publish` (trash, draft, private, any) on `/wp-json/wp/v2/posts` or `/wp-json/wp/v2/pages` returns `400 rest_invalid_param, "Status is forbidden"`. There is no way to see trashed content without WordPress admin credentials, which this system does not have and rule 5 (never publish, edit, or push to the live site) would prohibit using for a write action even if it did. This remains unreconciled and requires either Hassan or the user checking the trash directly.

## Cross-check against a report from a separate Claude Chrome extension, 2026-09-18

The user had a separate Claude-powered browser extension make direct edits to the live WordPress site, then pasted that extension's own written report of what it changed and found, asking for an independent cross-check against this system's tracker. The report was treated as an external claim to verify, not as a fact to accept. Every specific, checkable claim in it was tested directly against the live site (fresh JSON-LD fetch and parse, direct HTTP requests with no redirect-following, and a diff against this system's own dated historical record in this file) before being credited or refuted.

**Confirmed true, credited to the extension's edits:**

- **AS-001 (duplicate, non-matching Organization schema): fixed.** Both JSON-LD Organization blocks on the homepage now share the same `@id`, `https://www.albertscott.com/#organization`, confirmed by fetching the live page and parsing both blocks directly.
- **AS-002 (no `sameAs` in the company schema): fixed.** The schema now includes a `sameAs` array containing the company's LinkedIn and Crunchbase URLs, confirmed directly in the live JSON-LD.
- **A bonus fix not explicitly tracked as its own row:** `numberOfEmployees` is now unified at 30 across both Organization nodes (previously one said 5 to 10, the other 30). This falls out of the AS-001 merge rather than being a separate line item, but it is real and worth noting.
- **User enumeration via `/wp-json/wp/v2/users` is closed.** This endpoint now returns 404 rather than exposing account slugs. Confirmed by a direct request.
- **The `/marketing-division/` vs `/marketing-devision/` slug fix.** This is a genuine catch by the extension that this system's own tracker had backwards. As of today, `/marketing-division/` (correct spelling) is the live, canonical page (200, real content, correct title and H1), and the old misspelled `/marketing-devision/` now 301s to it. AS-036 previously recorded the homepage's link to `/marketing-division/` as a bug (on the assumption the misspelled URL was still canonical); that was wrong, and has been corrected in the tracker below.

**Confirmed inaccurate, with the evidence that contradicts each claim:**

- The report says `/blog/author/albertllc/` returns 404. A direct request shows it returns a 301 redirect to the homepage, not a 404. Both outcomes remove the page from search, so the practical effect is similar, but the report's specific mechanism is wrong.
- The report says `/?author=2` "lands on the homepage instead of redirecting to the username." A direct request shows it 301-redirects to `/blog/author/albertllc/`, meaning the account slug `albertllc` is still disclosed in the redirect's `Location` header. The user-enumeration vector through this specific parameter is not fully closed, contrary to the report's claim.
- The report says the thank-you pages had "no noindex at all yesterday." This system's own dated record in this file directly contradicts that: the 2026-09-14 re-verification pass already recorded a `noindex` robots directive on both thank-you pages, and the 2026-09-17 pass recorded that tag being simplified in wording (not added for the first time). The claimed baseline is at least three days stale.

**Still open, not addressed by either report:**

- **AS-004**, the AIOSEO schema block's description still reading "E-commerce Management" against the homepage's actual meta description, remains unfixed as of today's check. This is related to AS-001/AS-002 but is a distinct field on a distinct plugin setting; fixing the `@id` merge and `sameAs` did not touch it. Do not read AS-001/AS-002 being done as AS-004 also being done.
- **AS-253**, a live "9% Commission" pricing figure exposed inside broken shortcode text on `/services/`, found during this same pass (see below), is untouched by anything in the extension's report.

**Unverifiable from here:**

- The report references 20 additional rows, "AS-252 through AS-271," that are not present in this tracker or in the shared Google Sheet (which is byte-identical to the last pull). They may exist in a separate file this system has not been given access to. This system's own AS-252 (a homepage gallery block linking every tile to a redirecting case-studies URL, found 2026-09-16) is a different issue from the extension's own "AS-252" (described as a user-enumeration finding). This is a direct ID collision between two independently numbered lists; if the two are ever merged, one side's AS-252 will need renumbering to avoid confusion.

**New finding from this pass, not from either prior list:** while confirming the shortcode-broken legacy pages Hassan named as his reason for deleting AS-000, one of them, `/services/`, was found to expose a real pricing figure ("9% Commission", "9% Service") in its raw, unrendered shortcode text, visible to any visitor or crawler. Added as AS-253, priority P1, flagged for the user to confirm whether the figure is current or stale before any further action, given it sits inside content otherwise destined for the general legacy-page cleanup.

**Tracker changes made from this pass:** AS-001 and AS-002 marked Done with dated evidence; AS-004 marked explicitly still open (to prevent it being read as fixed alongside AS-001/AS-002); AS-036 corrected from an open finding to a resolved/stale finding, with the reasoning above; AS-037 (the "Explore Logistics" button still linking to `/logistics-division-2/`) reconfirmed still open, unaffected by the AS-036 correction; AS-253 added as a new row. Tracker grew from 252 to 253 rows. `issue-tracker.md`, `issue-tracker.xlsx`, and `issue-tracker-for-gsheet-import.xlsx` rebuilt from the corrected CSV, preserving the existing `DevNotes`/`Response`/`Complete` values already on the shared Google Sheet copy for every row except the new AS-253.

On the extension's closing question, whether to have it fix the two "confirmed homepage links": only one of those two is actually still broken. AS-036 (the `/marketing-division/` link) turned out to be a stale finding once verified, not a live bug, so there is only one real link left to fix, AS-037, the "Explore Logistics" button pointing at `/logistics-division-2/` instead of `/logistics-division/`. Whether to authorize another AI agent to make that edit directly on the live WordPress site is not this system's call; it is the user's, per rule 5 of this project (this system itself has neither WordPress write access nor the standing to grant it to another tool).

## Cross-check against the shared Google Sheet, 2026-09-21

Downloaded the shared Google Sheet directly (`issuetracker (1).xlsx`, file ID `1FgMacnZFDYNTTYmsPH86Mywr1Akj7-Y1`) to count how many of the 253 tracked issues the team considers resolved, and to check that count against this system's own live verification rather than trusting either source blindly.

**The sheet's own Status column, read as-is:** the sheet has 251 rows (AS-000 through AS-251, no AS-252 or AS-253), meaning it reflects a pull from on or before 2026-09-16 and has not picked up either row added since. Of those 251 rows, 118 carry a Status starting "Done" and 7 "Skipped". Every one of the 118 "Done" rows this system had itself previously annotated (with a "(verified YYYY-MM-DD: ...)" suffix) matches; the remaining "Done" rows in the sheet carry no verification note and appear to be self-reported check-offs by the team, not confirmed by this system before today.

**Cross-checking those team-reported "Done" rows against the live site today** turned up two different outcomes:

- **18 of them are now genuinely fixed, confirmed live today, and were stale/blank in this tracker until this pass:** AS-061 (marketing division slug and title corrected, confirmed by a direct fetch: title now reads "Marketing division - Albert Scott", no more "Devision"), AS-168 and AS-181 (the National Public Seatinf slug now 301s to the corrected "national-public-seating"), AS-182 (the Mouthwatchers blog post now 301s to its case study instead of duplicating it), AS-190 and AS-191 (`/home/` and `/amazon-management-agency/` now 301 to the homepage), AS-192, AS-193, and AS-194 (the three ad-landing pages now carry a noindex robots tag rather than sitting open in the index), AS-195 and AS-196 (both now return 404, the practical equivalent of the recommended delete), and AS-197 through AS-201, the five shortcode-broken legacy pages (`/services/`, `/services-2/`, `/our-service/`, `/additional-services/`, `/overview/`) that this system's own 2026-09-18 pass had just confirmed were still live and broken, all five now 301 to the homepage instead. This is a real, fast turnaround: fixed sometime between 2026-09-18 and today. **AS-253, the pricing figure exposed in `/services/`'s broken shortcode text, is resolved as a direct side effect of the AS-197 redirect:** that page no longer serves the shortcode content at all. Also confirmed via `/about-us/` directly: **AS-250 is fixed**, a genuine "Our Story" section is now present ("Albert Scott was founded in 2020 by David Greenblatt and Shlomo Greenblatt after seeing firsthand how difficult it had become for brands to succeed on Amazon..."), and the page grew from 648 to 721 words to hold it.
- **23 of them do not hold up against the live site and remain open:** AS-037 (already known stale in the sheet, reconfirmed still broken on 2026-09-18) and 22 of the 23 portfolio project pages (AS-158 to AS-167, AS-169 to AS-180; AS-168 and AS-181, the slug-typo row, are the two exceptions and are genuinely fixed, see above). A live spot check of 5 of these 22 (Superior Trading, Vianney, Rufus Teague, Nora, Blue Chill) shows each still has roughly 80 words of body copy, essentially just the boilerplate "Project Scope" line, and every image on every one of the 5 still has an empty or missing alt attribute. No content or alt-text work has actually happened on these pages; the sheet's "Done" marking for this block appears to be premature or a bulk check-off, not a real fix.

**Net result: 100 of the 253 tracked issues are confirmed resolved as of today**, up from 82 before this pass (18 newly confirmed above). This is lower than the sheet's own count of 118 "Done" because 23 of the sheet's "Done" rows do not hold up under a direct live check (22 thin/no-alt-text portfolio pages plus the already-known AS-037 regression). It is higher than the previous 82 because this pass caught a genuine, fast batch of real fixes (the shortcode-broken legacy pages, the ad-landing noindex work, and the about-us founding story) that had happened since the last check but that neither this tracker nor an untrusting read of the sheet alone would have surfaced without verifying against the live site directly.

One additional minor observation made while checking AS-061: the live site's own navigation menu still links to `/marketing-devision/` (the old misspelled slug) rather than the corrected `/marketing-division/`. It works, the 301 redirect covers it, so this is not blocking AS-061, but it is an unnecessary redirect hop present sitewide (the marketing division nav link appears in the shared header/footer). Not added as its own row given its low impact; noted here for visibility.

**Tracker changes made from this pass:** AS-061, AS-168, AS-181, AS-182, AS-190, AS-191, AS-192, AS-193, AS-194, AS-195, AS-196, AS-197, AS-198, AS-199, AS-200, AS-201, AS-250, and AS-253 all marked Done with dated evidence (18 rows). No rows added or removed; tracker stays at 253 rows. All three tracker files rebuilt from the corrected CSV.
