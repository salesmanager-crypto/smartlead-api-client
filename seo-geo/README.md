...
seo-geo/
  ...
  tracking/
    issue-tracker.csv / .xlsx / .md   one row per issue: category, page, issue, description, fix, priority, status
...
| Issue tracker (v1) | 2026-09-03 | `tracking/issue-tracker.csv`, `.xlsx`, `.md` | Flat checklist built from the audit: one row per issue with category or page, issue, what is happening, fix, priority, roadmap reference and a status column. |
| Issue tracker (v2, rebuilt per feedback) | 2026-09-03 | Same files, replaced | v1 summarized repeated findings as counts ("7 category archives", "23 project pages", "8 duplicate posts") without naming them. v2 gives every one of those its own row with its exact URL: 7 category archives, 2 tag archives, 10 project_category archives, 4 author archives named individually; all 23 portfolio projects named individually; all 32 triage pages named individually (was already close to this, now exact); the 8 "duplicate" blog posts re-checked against the actual project and case-study list, only 2 of them turned out to be real duplicates, corrected in place; 18 pages with em dashes listed individually. Added a URL column (clickable hyperlink in the xlsx). Grew from 151 to 239 rows. |
| Issue tracker re-verification | 2026-09-07 | Section appended to `audits/stage1-audit-2026-09-03.md`; tracker grew to 248 rows | Re-checked all 239 rows against the live site. None fixed. Found 2 new things since 2026-09-03: a new page (/brand-store-design/) and a new junk taxonomy (7 "dsm-attachment-category" archives), added as 9 new rows (AS-240 to AS-248). |
| Issue tracker re-verification | 2026-09-11 | Section appended to `audits/stage1-audit-2026-09-03.md`; tracker Status column updated; 249 rows (1 new urgent row added) | 26 rows confirmed fixed (6 duplicate pages deleted, 2 thank-you pages consolidated, em dashes removed from 14 pages), 1 partially fixed (DSP em dashes). Found 1 urgent regression: a real article was deleted, not one of the tracked issues; added as AS-000. |
| Issue tracker re-verification | 2026-09-14 | Section appended to `audits/stage1-audit-2026-09-03.md`; tracker grew to 251 rows; also produced `tracking/issue-tracker-for-gsheet-import.xlsx` | 69 rows confirmed fixed (all junk taxonomy archives noindexed, 3 dead links now redirect, 10 more pages correctly 301-redirected, homepage's redirect links fixed), 3 partially fixed. The 2026-09-11 regression (deleted article) is still not restored. Found this system's own Sep-14 automated check had 2 false "reopened" flags, both measurement artifacts, corrected. Found the Google Sheet the user asked to be updated cannot be written to with this session's tools (read/download only); a matching import-ready file was produced instead. |
| Issue tracker re-verification | 2026-09-16 | Section appended to `audits/stage1-audit-2026-09-03.md`; tracker grew to 252 rows; `issue-tracker-for-gsheet-import.xlsx` refreshed | Confirmed the Google Sheet had been updated externally to match the 2026-09-14 data. 10 more rows confirmed fixed. 2 rows found genuinely reopened on the homepage (confirmed by a direct HTML fetch, not a tool error): a logistics button and a marketing link both point at redirecting URLs again. 1 new problem found: a new homepage gallery block links every tile to a redirecting case-studies URL. AS-000 (the deleted article) still not restored, now 7 days. |
| Issue tracker re-verification | 2026-09-17 | Section appended to `audits/stage1-audit-2026-09-03.md` | Checked specifically for changes from a separate Claude Chrome extension the user had make edits. No tracker rows newly resolved. Two minor live-site changes found (author archives upgraded from noindex to a redirect; thank-you pages' robots tag simplified), neither matching the two WordPress-only fixes identified on 2026-09-16. AS-000 still not restored (8 days) and the two 2026-09-16 homepage regressions are both still present. Recorded Hassan's explanation for AS-000 from the email thread (intentional deletion, shortcode-broken content) alongside this system's conflicting 2026-09-03 evidence, unreconciled. |
...
| Cross-check vs. Claude Chrome extension report | 2026-09-18 | Section appended to `audits/stage1-audit-2026-09-03.md`; tracker grew to 253 rows; all three tracker files rebuilt | Independently verified a pasted third-party report of live edits made by a separate AI browser extension. Confirmed true and credited: AS-001 schema @id merge, AS-002 sameAs addition, numberOfEmployees unified, `/wp-json/wp/v2/users` closed to enumeration, and a genuine catch that this system's own AS-036 finding was stale (`/marketing-division/` is now the correct live slug). Confirmed inaccurate: the `/author/` 404 claim (actually a 301), the `/?author=2` claim (still discloses the username via redirect), and the thank-you-page noindex timeline (contradicted by this system's own dated Sep-14 record). AS-004 confirmed still open. Found one new issue (AS-253, a live pricing figure exposed in broken shortcode text on `/services/`). Flagged an ID collision between this tracker's own AS-252 and the extension's unrelated "AS-252." Also attempted a direct WordPress trash check for AS-000 at the user's request; confirmed this session has no path to non-public post statuses (401/400 on every attempt), so AS-000 remains unresolved from here. |
| Resolved-count cross-check vs. the shared Google Sheet | 2026-09-21 | Section appended to `audits/stage1-audit-2026-09-03.md`; 18 rows marked Done; tracker stays at 253 rows; all three tracker files rebuilt | Downloaded the shared Google Sheet directly and compared its Status column against a fresh live-site check for every row where the two disagreed. 18 of the sheet's "Done" claims were confirmed genuinely fixed today and updated in this tracker (5 shortcode-broken legacy pages now redirect, 3 ad-landing pages now noindexed, 2 pages now 404, a slug typo fixed, a duplicate post now redirects, the about-us founding story is now present, and the AS-253 pricing exposure is resolved as a side effect). 23 of the sheet's "Done" claims did not hold up: 22 portfolio project pages are still thin with no alt text despite being marked Done, and AS-037 remains the known regression. Net: 100 of 253 tracked issues confirmed resolved as of today, versus the sheet's own claimed 118 and this tracker's prior count of 82. |
